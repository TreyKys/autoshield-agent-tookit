...
import { SafeBoxArtifact } from "../contracts/SafeBox.js";
...
export class Surgeon {
  ...
  async performSurgery(targetAddress: string, cureCode: string): Promise<string> {
    ...
    // Deploy the "Safe Implementation" first
    console.log("👨‍⚕️ Preparing cure: Deploying Safe Implementation...");
    const safeImplementation = await deployContract({
      client: this.client,
      chain: HEDERA_TESTNET,
      account: this.account,
    ...
    console.log(`👨‍⚕️ Safe Implementation deployed at: ${safeImplementation}`);
