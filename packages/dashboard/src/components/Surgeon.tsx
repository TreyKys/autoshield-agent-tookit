...
const client = createThirdwebClient({
  clientId: import.meta.env.VITE_THIRDWEB_CLIENT_ID || "demo", // Should come from env
});
...
export function Surgeon({ onComplete }: SurgeonProps) {
  ...
  const executeSurgery = async () => {
    ...
    try {
      ...
      const targetAddress = import.meta.env.VITE_TARGET_ADDRESS;
      if (!targetAddress) {
        throw new Error("No Target Address found. Please set VITE_TARGET_ADDRESS.");
      }
